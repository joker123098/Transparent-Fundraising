...
(use-trait coreTrait 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.citycoin-core-trait.citycoin-core)
...
(define-public (activate-token (coreContract principal) (stacksHeight uint))
  (let
    (
      (coreContractMap (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.miamicoin-auth get-core-contract-info coreContract)))
    )
...
(define-public (burn (amount uint) (recipient principal))
  (let
    (
      (coreContract (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.miamicoin-auth get-core-contract-info contract-caller)))
    )
