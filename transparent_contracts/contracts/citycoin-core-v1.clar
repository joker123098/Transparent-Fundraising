  ...
  (let
    ...
    (if (is-eq newId threshold)
      (let
        ...
        (try! (contract-call? .miamicoin-auth activate-core-contract (as-contract tx-sender) activationBlockVal))
        (try! (contract-call? .miamicoin-token activate-token (as-contract tx-sender) activationBlockVal))
...
(define-private (stack-tokens-at-cycle (user principal) (userId uint) (amountTokens uint) (startHeight uint) (lockPeriod uint))
  (let
    ...
    (try! (contract-call? .miamicoin-token transfer amountTokens tx-sender (as-contract tx-sender) none))
...
(define-private (claim-stacking-reward-at-cycle (user principal) (stacksHeight uint) (targetCycle uint))
  (let
    ...
    (if (> toReturn u0)
      (try! (as-contract (contract-call? .miamicoin-token transfer toReturn tx-sender user none)))
      true
    )
...
(define-private (set-coinbase-thresholds)
  (let
    (
      (coinbaseAmounts (try! (contract-call? .miamicoin-token get-coinbase-thresholds)))
    )
...
(define-private (mint-coinbase (recipient principal) (stacksHeight uint))
  (as-contract (contract-call? .miamicoin-token mint (get-coinbase-amount stacksHeight) recipient))
)
